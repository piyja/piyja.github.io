...
3. The user experience for invehicle can be greatly enhanced by leveraging edge models with input from the cloud in form of compute, tools or peer agents. This blog talked about the hybrid cloud-edge architecture — Local agents for in-car tasks, cloud agents for web/trip planning; key challenges are intent routing, context sharing, and graceful degradation when the vehicle is offline.
...
5. For the OEMs, Nvidia comes with a hardware solution in 3 levels each with more commitment for AI ready hardware stack: 
    ...
    * A single central car computer with DRIVE AGX fused with the MediaTek Dimensity AX SoC - a fully integrated solution for infotainment and AI workloads, but would require more commitment to the new hardware architecture.

### Critical focus areas for building in-vehicle AI agents

1. **Specialised models for the voice pipeline** — 
    The 400-500ms end-to-end SLA is not a single constraint; it is a budget distributed across every stage of the pipeline. Each arrow in `Wake Word → ASR → Orchestrator → LLM → TTS → Audio Output` carries a latency cost, leaving roughly 50-100ms per stage — which immediately constrains model size and quantization decisions at every step.
...
2. **Platform scalability and the standardisation gap** — 
    No current automotive software standard addresses agentic AI orchestration. AUTOSAR Adaptive has introduced ML model management interfaces, but nothing covers agent-to-agent communication contracts, tool calling interfaces, memory management for multi-turn context, or evaluation criteria for agentic behaviour. AGL and OEM-specific frameworks share the same gap. This is the most significant structural obstacle to long-term scalability of in-vehicle AI agents.
...
3. **Agents deep in the automotive software stack** 
    ...
    The core tension is this: LLM-powered flexibility is inherently non-deterministic, while Classic AUTOSAR at ASIL-D requires hard real-time determinism. 
...
2. **Personalised and context-aware setup** — 
